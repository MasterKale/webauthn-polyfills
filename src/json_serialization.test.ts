...
describe('PublicKeyCredential.parseCreationOptionsFromJSON', () => {
  ...
  const expected = {
    ...original,
    challenge: Base64URL.decode(original.challenge),
    user: {
      ...original.user,
      id: Base64URL.decode(original.user.id),
    },
    excludeCredentials: original.excludeCredentials?.map((excludeCredential) => {
      return {
        ...excludeCredential,
  ...
  const target = parseCreationOptionsFromJSON(original as PublicKeyCredentialCreationOptionsJSON);
  it('should parse CreationOptions', () => {
    // @ts-ignore Temporarily avoid type mismatch
